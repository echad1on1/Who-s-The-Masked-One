...
// ----------------------------------------------------
// MASK CHECK: if player has mask on, freeze & ignore.
// Swap this with your real flag once you confirm it.
// ----------------------------------------------------
var mask_on = false;

// Prefer a player instance var if it exists
if (variable_instance_exists(plr, "mask_on")) mask_on = plr.mask_on;

// Fallbacks if you store it elsewhere
else if (variable_global_exists("mask_on")) mask_on = global.mask_on;
else if (variable_global_exists("player_mask_on")) mask_on = global.player_mask_on;

// If masked: no aggro, no movement, cancel attacks
if (mask_on) {
    vx = 0; vy = 0;
    state = ST_IDLE;
    timer = 0;
...
if (mask_on) {
    ...
    // (Optional) still collide-resolve if you ever get pushed into walls
    // var v0 = scr_move_and_collide(0, 0, solid_obj);
